...
class InvestorAuthProvider with ChangeNotifier {
  ...
  Future<void> _createOrUpdateUserRecord(InvestorUser user) async {
    try {
      debugPrint('🔍 Creating/updating investor user record for: ${user.id}');

      // CRITICAL: Check if user record exists - Filter by specific user ID
      final existingUser =
          await _supabase
              .from('investors') // This is the INVESTOR table
              .select('id, email, created_at')
              .eq('id', user.id) // THIS IS THE KEY FIX
              .maybeSingle();

      if (existingUser == null) {
        // Create new INVESTOR user record
        ...
        _logger.i(
          '✅ Created new INVESTOR user record for: ${user.email} with ID: ${user.id}',
        );
        debugPrint('✅ Created INVESTOR user record for ID: ${user.id}');
      } else {
        // Update existing INVESTOR user record
        final updateData = {
          'last_login_at': user.lastLoginAt.toIso8601String(),
          'is_verified': user.isVerified,
          'updated_at': DateTime.now().toIso8601String(),
        ...
        _logger.i(
          '✅ Updated INVESTOR user record for: ${user.email} with ID: ${user.id}',
        );
        debugPrint('✅ Updated INVESTOR user record for ID: ${user.id}');
      }
    } catch (e) {
      _logger.e('❌ Error creating/updating INVESTOR user record: $e');
      debugPrint('❌ Error with INVESTOR user record for ID: ${user.id}');
  ...
  Future<bool> validateInvestorUserAccess() async {
    try {
  ...
  Future<void> checkAuthState() async {
    try {
      ...
      if (user != null) {
        ...
        _currentUser = InvestorUser.fromSupabaseUser(user);
        _isLoggedIn = true;
